...
export const RecommendedOpportunities = ({ refreshTrigger }) => {
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        {[1, 2].map((i) => (
          <div key={i} className="animate-pulse bg-secondary/20 p-4 rounded-[var(--radius-md)] border border-border">
  ...
  if (error) {
    return (
      <div className="p-4 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-[var(--radius-md)] text-sm border border-red-100 dark:border-red-900/50">
        {error}
      </div>
    );
  }
      ...
      {recommendations?.filter(Boolean)?.slice(0, 3).map((rec, index) => {
        ...
        return (
          <div 
            ...
            className="group relative bg-card p-4 rounded-[var(--radius-md)] border border-border/50 hover:border-primary/50 hover:shadow-md transition-all cursor-pointer overflow-hidden"
          >
            {/* AI Match Badge */}
            ...
            {matchedSkills && matchedSkills.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {matchedSkills.slice(0, 3).map((skill, i) => (
                  <span key={i} className="text-[10px] uppercase font-bold tracking-wider bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-[var(--radius-sm)]">
                    {skill}
                  </span>
                ))}
